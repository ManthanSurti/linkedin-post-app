// Generates 7 LinkedIn posts from ONE topic — one per angle, all in parallel.
// Uses Gemini 2.5 Flash (fast, fits in Netlify timeout).
// 7 parallel calls stay safely under the 10 RPM free-tier limit.
const CORS = {
...
exports.handler = async (event) => {
  ...
  // Fire all 7 in parallel — Flash is fast and 7 requests fits under 10 RPM.
  ...
  const results = ANGLES.map((angleInfo, i) => {
    ...
    return { day: angleInfo.day, angle: angleInfo.angle, error: outcome.reason?.message || 'Unknown error' };
  });
